...
# Generate Emissions
def generateEmissions(AREA_em, LINE_em, RLINEXT_em, VOLUME_em, isLink, em_rd_list, GISRdID, VOLUME_file_path, outputpath):

    # Inner join tables rd_list and emission_table on linkID
    #df = pd.merge(rd_list, em_df, how='inner', on=[GISRdID])
    ...
    em_rd_list['length_m'] = em_rd_list['geometry'].length
    if isLink.get():
        em_rd_list['emrate'] = em_rd_list['Emission rate']
    else:
        em_rd_list['emrate'] = em_rd_list['Emission rate'] * (em_rd_list['length_m'] * 0.000621371)
    ...
    if AREA_em.get():
        ...
        df = pd.merge(df, em_rd_list[[GISRdID, 'emrate']], how='left', on=[GISRdID])
