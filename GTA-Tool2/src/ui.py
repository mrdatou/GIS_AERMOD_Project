...
# Draw Tab class inherit Data class to share data with other tabs
class DrawTab(ttk.Frame, Data):
    ...
    def createFrame(self):
        ...
        self.button_update.configure(text="Update graph", padding=10, default=tk.ACTIVE, width=15,
                                     command=self.btnUpload)
        self.button_update.grid(padx=10, pady=10, row=0, column=0, sticky=tk.NW)

        '''
        self.color_frame = tk.Frame(self, bg="red")
        self.color_frame.configure(bg='red')
        self.color_frame.grid(row=0, column=1)
        tk.Label(self.color_frame, text="AAA", bg="red").grid()
        '''

        # Image frame
        # Buttons
        self.img_frame = ttk.Frame(self, width=500, height=500)
        self.img_frame.grid(row=1, column=0, sticky="nswe")
        tk.Grid.rowconfigure(self.img_frame, 0, weight=1)
        tk.Grid.columnconfigure(self.img_frame, 0, weight=1)

        # Buttons
        self.btn_frame = ttk.Frame(self)
        self.btn_frame.grid(row=1, column=1)
        tk.Grid.rowconfigure(self.btn_frame, 0, weight=1)
        tk.Grid.columnconfigure(self.btn_frame, 0, weight=1)

        self.button_scale = ttk.Button(self.btn_frame)
        self.button_scale.configure(text="Define scale", padding=10, width=15, default=tk.ACTIVE, command=self.btnScale)
        self.button_scale.pack(padx=10, pady=10)

        self.button_ref = ttk.Button(self.btn_frame)
        self.button_ref.configure(text="Locate reference", padding=10, width=15, default=tk.ACTIVE,
                                  command=self.btnLocRef)
        self.button_ref.pack(padx=10, pady=10)

        self.button_draw = ttk.Button(self.btn_frame)
        self.button_draw.configure(text="Draw polyline", padding=10, width=15, default=tk.ACTIVE,
                                   command=self.btnDrawPoly)
        self.button_draw.pack(padx=10, pady=10)

        self.button_confirm = ttk.Button(self.btn_frame)
        self.button_confirm.configure(text="Confirm", padding=10, width=15, default=tk.ACTIVE,
                                      command=self.btnConfirm)
        self.button_confirm.pack(padx=10, pady=10, side=tk.BOTTOM)

    def btnUpload(self):
        path = askopenfilename(title='Select an image', initialdir=os.getcwd())
        ...
        try:
            ...
            self.btnUpload()  # try to open new image again
            return
    ...
    def setCanvas(self):
        if self.canvas is not None:
            self.canvas.destroy()
            self.canvas.sheet.destroy()

        self.canvas = Polygons(self.img_frame, self.img_path, (320, 240))
        self.canvas.grid(row=1, column=0, sticky=tk.N + tk.S + tk.W + tk.E)

    # Define scale button
    def btnScale(self):
        """ Draw line -> Second point selected -> keep the coordinates -> popup window to define scale"""
        if self.canvas is None:
            messagebox.showwarning('Warning', 'Select image first')
            return

        self.canvas.isScale = True
        self.canvas.isLocRef = False
        self.canvas.isDrawPoly = False
        pass
    ...
    def btnLocRef(self):
        ...
        self.canvas.isScale = False
        self.canvas.isLocRef = True
        self.canvas.isDrawPoly = False

    def btnDrawPoly(self):
        if self.canvas is None:
            messagebox.showwarning('Warning', 'Select image first')
            return
        ...
        self.canvas.isScale = False
        self.canvas.isLocRef = False
        self.canvas.isDrawPoly = True

    def btnConfirm(self):

        Data.xref_left_m, Data.xref_right_m, Data.yref_higher_m, Data.yref_lower_m, gdf = self.canvas.convertData()
        Data.rd_list = gdf

        # Set columns
        Data.roadID = "linkID"  # Road ID
        Data.roadTp = "rdTypeID"  # Road Type
        Data.geom = "geometry"  # Geometry

        # Make directory for generated files
        dir_name = os.path.dirname(self.img_path)
        fd_name = Path(self.img_path).stem

        if not os.path.exists(dir_name + '/' + fd_name):
            os.makedirs(dir_name + '/' + fd_name)
            os.makedirs(dir_name + '/' + fd_name + '/results')

        Data.output_path = dir_name + '/' + fd_name + '/results'

        # Set Data tab values
        #self.master.master.data_tab.rd_list = gdf
        #self.master.master.data_tab.openFromDrawTab()
...
class DataTab(tk.Frame, Data):
    ...
    # Set values from Draw Tab
    def openFromDrawTab(self):
        self.buildDropDownMenuSetting()
        self.isOpenSecond.set(True)
...
class ReceptorsTab(tk.Frame, Data):
    ...
    def btnGenerateRec(self):
        ...
        if self.checkInput():
            ...
            # Transform all data except F_LTYPE to int
            df = df.astype({'F_LTYPE': 'string', 'LAYER': 'int', 'DISTANCE': 'int', 'INTERVAL': 'int'})
...
class EmissionsTab(tk.Frame, Data):
    ...
    def createEmissionConvFrame(self):
        ...
        tk.Label(self.emission_conversion_frame, text="Input for Emission Conversion", wraplength=300,
                 font=('', 12, 'bold')).grid(row=1,
                                             column=0,
                                             columnspan=2)
        tk.Label(self.emission_conversion_frame, text="1. Type or upload link emissions",
        ...
        # Spreadsheet
        self.emission_sheet = EmSheet(None, self.emission_conversion_frame)
        self.emission_sheet.grid(row=3, column=0, rowspan=3, sticky=tk.W)

        tk.Label(self.emission_conversion_frame, text="Or upload csv", font=self.label_font).grid(row=3, column=1)

        # Upload emission csv button
        self.button_upload_em = ttk.Button(self.emission_conversion_frame)
        self.button_upload_em.configure(text="Upload emission csv", padding=10, default=tk.ACTIVE, command=self.btnUpEm)
        self.button_upload_em.grid(row=3, column=2, sticky=tk.W, padx=10, pady=10)
        ...
        self.chk_mile.grid(row=4, column=2)
        ...
        self.chk_area.grid(row=8, column=0, sticky=tk.W)
        ...
        self.chk_line = tk.Checkbutton(self.emission_conversion_frame, variable=self.bln_line, text="LINE or RLINE",
                                       font=self.label_font)
        self.chk_line.grid(row=9, column=0, sticky=tk.W)
        ...
        self.chk_rlinext = tk.Checkbutton(self.emission_conversion_frame, variable=self.bln_rlinext, text="RLINEXT",
                                          font=self.label_font)
        self.chk_rlinext.grid(row=10, column=0, sticky=tk.W)
        ...
        self.chk_vol = tk.Checkbutton(self.emission_conversion_frame, variable=self.bln_vol, text="VOLUME",
                                      font=self.label_font)
        self.chk_vol.grid(row=11, column=0, sticky=tk.W)
        ...
        self.button_volume.grid(row=11, column=1, sticky=tk.W, padx=10, pady=10)
        ...
        self.button_generateEm.grid(row=12, column=0, columnspan=2, padx=10, pady=10)

    # Control Skip check button
    def controlChkSkip(self):
        # If true, disable all features
        if self.bln_skip.get() == True:
            self.button_upload_em.config(state=tk.DISABLED)
            ...
            self.button_upload_em.config(state=tk.NORMAL)
    ...
    # Upload emission csv button control
    def btnUpEm(self):
        pathEmcsv = tk.filedialog.askopenfilename(initialdir=Data.output_path, title="Select file",
                                                  filetypes=(("csv files", "*.csv"), ("all files", "*.*"))
                                                  )

        # Import csv file
        csv_df = pd.read_csv(pathEmcsv)

        if self.emission_sheet is not None:
            self.emission_sheet.destroy()

        self.emission_sheet = EmSheet(csv_df, master=self.emission_conversion_frame)
        self.emission_sheet.grid(row=3, column=0, rowspan=3, sticky=tk.W)
    ...
    # Verify input and generate emission button
    def btnEm(self):
        output_list = self.emission_sheet.get_sheet_data(return_copy=False, get_header=False, get_index=False)

        self.em_df = pd.DataFrame(output_list)

        self.em_df = self.em_df.iloc[:, 0:2]
        self.em_df.columns = ["linkID", "Emission rate"]

        self.em_df = self.em_df.replace('', np.nan, regex=True)
        self.em_df = self.em_df.dropna(how='all')
        ...
        if self.checkInput():
            ...
            # If any row is deleted because no corresponding linkID in emission df
            if self.deleted_row is not None:
                if len(self.deleted_row) > 0:
                    print("info msg")
                    delete_row_msg = "The following linkID is deleted:"

                    for row in self.deleted_row:
                        delete_row_msg += " linkID-" + row

                    messagebox.showinfo('Deleted rows', delete_row_msg)
    ...
    def checkInput(self):
        ...
        # Check if spreadsheet has missing value
        if self.em_df.isnull().values.any():
            errFlag = False
            errMsg += "\nThere are missing values in the table"

        # Check spreadsheet values
        try:
            self.em_df = self.em_df.astype({"linkID": str, "Emission rate": float})
        except ValueError:
            errFlag = False
            errMsg += "\nEmission rate should be float"
    ...
    def generateEmissions(self):
        ...
        em_rd_list = pd.merge(Data.rd_list, self.em_df, how='inner', on=Data.roadID)

        self.deleted_row = Data.rd_list[Data.roadID][~Data.rd_list[Data.roadID].isin(self.em_df[Data.roadID])].to_list()

        generateEmissions(self.bln_area, self.bln_line, self.bln_rlinext, self.bln_vol, self.bln_link, em_rd_list,
                          Data.roadID, self.pathVolcsv,  Data.output_path)
        pass


# Emission tab tksheet
class EmSheet(tksheet.Sheet):
    def __init__(self, emission_df, master=None):
        super().__init__(master, column_width=150, height=200)
    ...
    def createTable(self):
        ...
        # List of output table
        if self.emission_df is None or len(self.emission_df) == 0:
            lst_df = [["", ""]]
        else:
            lst_df = self.emission_df.values.tolist()

        # Set header
        headers_list = EmissionsTabConstants.colList
        headers = [f'{c}' for c in headers_list]
        self.headers(headers)
...
class MainWindow(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        # Add Draw tab
        self.draw_tab = DrawTab(master=self.nb)
        self.nb.add(self.draw_tab, text=' Draw ', padding=5)
